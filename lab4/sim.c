...
static void insn_sad(uint32_t insn)
{
	int opa = get_opa(insn);
	int regd = get_dreg(insn);

	int pixel1_addr = sr_read(0) + opa;
	int pixel2_addr = sr_read(1) + opa;
	
	uint16_t pixel1 = mem0_read(pixel1_addr);
	uint16_t pixel2 = mem1_read(pixel2_addr);

	uint16_t this_iteration = abs16(pixel1 - pixel2);
	uint16_t old_acc = sr_read(31);
	uint16_t result = old_acc + this_iteration;

	// Write back to sr31
	sr_write(31, result);
	
	// Compare to old best
	uint16_t old_best = sr_read(30);
	if(result >= old_best) {
		repeat_sad_stop(5);
	}

	// Write into rd
	set_reg(regd, result, 4);
}
